package com.primeclient.commands;
...
public final class PrimeCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(
        ...
        if (args.length == 0) {
            ...
            if (!player.hasPermission("primeclient.use")) {
        ...
        switch (sub) {
            case "reload" -> {
                if (!sender.hasPermission("primeclient.reload") && !sender.hasPermission("primeclient.admin")) {
                    sender.sendMessage("No permission.");
                    return true;
                }
                plugin.reloadAll();
                sender.sendMessage("PrimeClient reloaded.");
            }
            case "info" -> {
                if (!sender.hasPermission("primeclient.info") && !sender.hasPermission("primeclient.admin")) {
                    sender.sendMessage("No permission.");
                    return true;
                }
                if (args.length < 2) {
