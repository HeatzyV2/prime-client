package com.primeclient.detection;
...
/**
 * Detects Prime Client via {@code primeclient:main} handshake.
...
public final class ClientDetectionService implements PluginMessageListener {
    ...
    public static final String CHANNEL = "primeclient:main";
    ...
    public ClientDetectionService(PrimeClientPlugin plugin, Database database) {
        this.plugin = plugin;
    ...
    private void handleHandshake(Player player, JsonObject obj) {
        ...
        boolean validClient = "Prime Client".equalsIgnoreCase(client.trim());
        ...
        if (!validClient || !validProtocol || !validUuid) {
            ...
            reject.addProperty("message", plugin.getConfig().getString(
                    "protocol.reject-message", "Unsupported Prime Client protocol"));
        ...
        database.markPrimeClient(player.getUniqueId(), player.getName(), version, now);
        ...
        if (plugin.getConfig().getBoolean("verified.grant-permission", true)) {
            player.addAttachment(plugin, "primeclient.verified", true);
        }

        JsonObject accept = new JsonObject();
        accept.addProperty("t", "SERVER_ACCEPTED");
        accept.addProperty("message", plugin.getConfig().getString(
                "protocol.accept-message", "Welcome — Prime Client verified"));
        sendJson(player, accept);

        sendAccountSync(player);

        Bukkit.getScheduler().runTask(plugin, () ->
                Bukkit.getPluginManager().callEvent(new PrimeClientDetectedEvent(player, version, first)));
    }
}
