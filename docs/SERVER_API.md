# Prime Client Server API

Custom payload channel for partner Minecraft servers to detect and integrate with **Prime Client**.

| | |
|--|--|
| Channel | `primeclient:main` |
| Protocol | `1` |
| Transport | Fabric `CustomPacketPayload` / plugin messaging |
| Direction | C2S handshake + profile · S2C sync / XP / rewards / notify |

> Handshake proves the player runs Prime Client. **Do not trust the client for grants, economy, or permissions** — always validate rewards server-side.
...
Prime Client pins partner entries in the multiplayer list (non-removable):
...
- Minecraft **1.21.x** and **26.2** Prime Client jars
