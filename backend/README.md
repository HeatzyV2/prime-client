...
**Version:** `2.1.6` â€” SQLite persistence, MS profile verify (optional), party invites, friend notes, block list, profiles, Prime Coins store, cosmetics/settings sync, crash index, **public download/launch stats**.
...
After pulling `2.1.6`, restart the process once so SQLite migrates (`usage_stats` / `stats_dedupe` tables).
...
| Path | Role |
|------|------|
| `GET /health` | `{ version: "2.1.6", db, ws }` |
...
7. **Health** â€” `curl http://127.0.0.1:26005/health` â†’ `version` `2.1.6`, `db.ok` true.
