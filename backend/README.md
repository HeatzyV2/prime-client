...
**Version:** `2.1.2` — SQLite persistence, MS profile verify (optional), party invites, friend notes, block list, profiles, Prime Coins store, cosmetics/settings sync, crash index, **public download/launch stats**.
...
After pulling `2.1.2`, restart the process once so SQLite migrates (`usage_stats` / `stats_dedupe` tables).
...
| Path | Role |
|------|------|
| `GET /health` | `{ version: "2.1.2", db, ws }` |
...
7. **Health** — `curl http://127.0.0.1:26005/health` → `version` `2.1.2`, `db.ok` true.
